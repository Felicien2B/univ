...
public class Jeu {
	 ...
	 * @param nbEtapes Nombre de déplacements à réaliser par chacun des personnages
	 * @param nbObstacles Nombre maximum d'obstacles présents
	 */
	...
	public void lancerJeu() {
		...
		for (int nEtape=0; nEtape<nbEtapes; nEtape++) {
			for (Personnage perso:listePersos) {
				...
				if (posSouhaitee >= NBCASES) //Si la position souhaitée dépasse le tableau, on la remplace par la dernière case
					posSouhaitee = NBCASES-1;
				Case caseSouhaitee = cases[posSouhaitee];
				if (caseSouhaitee.estLibre()) {
					cases[perso.getPosition()].enleverPersonnage(); //On enlève le personnage de sa position
					caseSouhaitee.placerPersonnage(perso); //On le place sur sa nouvelle position
					perso.deplacer(posSouhaitee, caseSouhaitee.getGain()); //On garde la nouvelle position en mémoire et on ajoute le gain au joueur
				}
				else {

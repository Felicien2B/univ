...
Date : 20/09/2021
Version : 1
Description : Exercices de la partie 2 de l'atelier 5
"""
...
def perf_mix(f_mix, f_shuffle: callable,
             lst_tailles: [int], executions: int)-> ([float], [float]):
    """
    Calcule le temps d'exécution moyen de deux fonctions de mélange
    pour une même liste

    inputs:
        f_mix, f_shuffle: callable : Fonctions de mélange de liste
        lst_tailles: [int] : Liste des différentes tailles de liste
                             utilisées pour la comparaison
        executions: int : Nombre moyen d'exécutions
...
executions = [10,100,1000,10000,100000]
print(perf_mix(mix_list, rd.shuffle, executions, 10))